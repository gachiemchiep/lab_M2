...
def check_argv(argv_):
    if not (len(argv_) == 5):
        print "Usage python %s OFs_dir, stacked_count, output_hdf5, crop/noncrop" % (argv_[0])
        sys.exit(0);
    else:
        OFs_dir = argv_[1];
...
# OF_x_paths_: -> index:  1,...,10  15,..,25 ( in case of 10 stacked_count)
...
def main():
    ...
    sampling_start = 3;
    sampling_dist = int(
        (frame_count - 10 - 3) / 25);  # fixed sized, so only spatial can be used with 10opflows, 1opflows, 5opflows
