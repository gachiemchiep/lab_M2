...
#### global params ####
MODES = ("image", "opflows","opflows_crop")
THRESHOLD_VAL = 64;
...
def check_argv(argv_):
    if (len(argv_) < 4):
        ...
        print "Usage: python list_2_leveldb.py h5list output_leveldb image\n" \
              ...
              "Or \n" \
              "python list_2_leveldb.py h5list output_leveldb opflows_crop shuffled/unshuffled threshold/raw"
...
def merge_imgs(argv_):
    ...
    im_1 = cv2.imread(patch_1_paths[0], cv2.COLOR_RGB2BGR);
    print im_1.shape
    im_1_datum = im_1.transpose(im_1.shape[2], im_1.shape[0], im_1.shape[1]); # h x w x channel -> channel x h x w
    ...
    for merge_time in range(merge_time_count):
        ...
        for list_index in range(list_start, list_end):
            im_1 = patch_1_paths[list_index];
            im_2 = patch_2_paths[list_index];
            similarity = similarities[list_index];

            # im_1_gray = cv2.imread(patch_1_paths[list_index], cv2.IMREAD_GRAYSCALE);
            # im_2_gray = cv2.imread(patch_2_paths[list_index], cv2.IMREAD_GRAYSCALE);

            im_1 = cv2.imread(patch_2_paths[list_index],  cv2.COLOR_RGB2BGR);
            im_2 = cv2.imread(patch_2_paths[list_index],  cv2.COLOR_RGB2BGR);

            im_1_datum = im_1.transpose(im_1.shape[2], im_1.shape[0], im_1.shape[1]);
            im_2_datum = im_2.transpose(im_2.shape[2], im_2.shape[0], im_2.shape[1]);
...
def merge_opflows_crop(argv_):
    ...
    for merge_time in range(merge_time_count):
        ...
        for list_index in range(list_start, list_end):
            ...
            data_1[count_start:count_end, :, :, :] = h5_1_data[:, :, :, 42:298];
            data_2[count_start:count_end, :, :, :] = h5_2_data[:, :, :, 42:298];
            data_sims[count_start:count_end] = similarity;
...
ACTIONS = {'image': merge_imgs, 'opflows': merge_opflows, 'opflows_crop': merge_opflows_crop};
