...
int main(int argc, char** argv){
	...
	const char* keys =
		{
			...
			"{ l  | of_length | 10  | length of optical flows}"
		};
    ...
    int of_length = cmd.get<int>("pf_length");
    ...
    // only calculate for 25 part
    int sampling_distance = floor( (frame_count - of_length) / 25);
    std::cout << sampling_distance << std::endl;
	...
	while(true) {
        ...
        sampling_distance_count ++;
        if (sampling_distance_count > sampling_distance) {
            for (int i = 0; i < sampling_distance; i++) {
                capture >> frame;
                frame_num ++;
                std::cout << frame_num << " ";
            }
